...
### Exercise 3.1: Vercel Cloud Deployment & Check Links
Ứng dụng Next.js Frontend và Backend được deploy gộp chung thành công trên nền tảng Vercel:

* **Giao diện Chat Frontend (Auto-Mock):** [https://railway-tau.vercel.app/u/0/app](https://railway-tau.vercel.app/u/0/app)
* **Backend Health Probe:** [https://railway-tau.vercel.app/health](https://railway-tau.vercel.app/health)
* **Backend Ready Probe:** [https://railway-tau.vercel.app/ready](https://railway-tau.vercel.app/ready)
* **API Version Log check:** [https://railway-tau.vercel.app/api/version-log](https://railway-tau.vercel.app/api/version-log)
* **API Eval Cases check:** [https://railway-tau.vercel.app/api/eval-cases](https://railway-tau.vercel.app/api/eval-cases)
...
- **API key được check ở đâu?**
  - API key được kiểm tra tại hàm dependency `verify_api_key()` (dòng 100-106 trong `06-lab-complete/app/main.py`). Hàm này được định nghĩa như một dependency của FastAPI và được inject trực tiếp vào các endpoint qua dependency injection: `_key: str = Depends(verify_api_key)`.
...
- **Điều gì xảy ra nếu sai key?**
  - Nếu thiếu API key hoặc truyền sai API key, ứng dụng sẽ trả về mã lỗi HTTP **401 Unauthorized** kèm thông điệp: `"Invalid or missing API key. Include header: X-API-Key: <key>"`.

- **Làm sao rotate key?**
  - Khóa bảo mật được tải động từ biến môi trường `AGENT_API_KEY` (thông qua `os.getenv("AGENT_API_KEY")` hoặc từ file config).
...
- **Algorithm nào được dùng?**
  ...
  - Mỗi API key có một hàng đợi double-ended queue (`deque`) chứa các timestamp của các request gần nhất. Khi có request mới, các timestamp cũ vượt quá cửa sổ trượt (60 giây) sẽ bị loại bỏ khỏi hàng đợi (`window.popleft()`), sau đó độ dài của hàng đợi được so sánh với giới hạn để quyết định xem có cho phép request đi tiếp hay không.

- **Limit là bao nhiêu requests/minute?**
  - Giới hạn được cấu hình động qua cài đặt `rate_limit_per_minute` (mặc định là **60 requests/minute**).
...
* **Liveness Probe (`/health`)**: Trả về trạng thái hoạt động tổng thể của tiến trình (`status: ok`).
* **Readiness Probe (`/ready`)**: Kiểm tra xem ứng dụng đã sẵn sàng xử lý traffic chưa. Trả về mã lỗi `503 Service Unavailable` nếu biến trạng thái `_is_ready` là `False` (đang khởi động hoặc đang tắt). Load balancer sẽ dừng định tuyến traffic tới instance này cho tới khi nó sẵn sàng.
...
* Sử dụng vòng lặp kiểm tra biến `_in_flight_requests` để đợi các request hiện tại đang xử lý dở dang được hoàn tất (tối đa 30 giây) trước khi đóng các kết nối sạch sẽ.

#### 3. Stateless Design (Thiết kế phi trạng thái)
* Không lưu trữ lịch sử trò chuyện (`conversation_history`) hay session trong RAM của container.
* Sử dụng Redis làm cơ sở dữ liệu tập trung lưu trữ trạng thái. Các câu lệnh được dùng để đọc/ghi lịch sử hội thoại của user. Nhờ đó, bất cứ instance nào sau Load Balancer đều có thể xử lý request tiếp theo của user mà không bị mất dữ liệu ngữ cảnh.
* **Tại sao khi scale ra nhiều instances ta không nên dùng memory cục bộ để lưu lịch sử hội thoại?**
...
* **Cải tiến đặc biệt (Auto-Mock Mode)**:
  * **Full Auto-Mock Mode**: Đã chuyển cấu hình Frontend và toàn bộ Next.js API Routes sang Mock Mode hoàn toàn. Các API như `/api/eval-cases`, `/api/prompt-tools`, `/api/version-log` tự động đọc file cục bộ từ repository bằng Node.js `fs` module, giúp app chạy ổn định độc lập 100% trên Vercel mà không phụ thuộc vào FastAPI chạy thực tế.
  * **Vercel Co-location**: Tích hợp Frontend Next.js và Backend FastAPI chạy gộp chung trên Vercel Lambda.

### Exercise 6.2: CI/CD Pipeline
* **Thiết lập CI/CD**:
  * **Pipeline CI (`.github/workflows/ci.yml`)**: Tự động kích hoạt khi có Push hoặc Pull Request vào nhánh `main`. Quy trình kiểm thử toàn diện gồm:
    ...
    4. Chạy `check_production_ready.py` kiểm tra 20/20 tiêu chuẩn Production.
  * **Tạm dừng Docker Job**: Đã vô hiệu hóa Docker build trong GitHub Actions workflow (`ci.yml` và `deploy-docker.yml`) để tăng tốc độ CI và giảm tải tài nguyên.
  * **Pipeline CD Vercel (`.github/workflows/deploy-vercel.yml`)**: Tự động build và deploy dự án lên Cloud Vercel.
